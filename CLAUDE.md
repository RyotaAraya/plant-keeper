...
- **発注の受領 → 入庫**: 発注を受領済にするには入庫先の倉庫（`orders.warehouse_id`）が必要。受領すると、同じロット（資材・倉庫・受領日が同じ）に数量を足すか新しい在庫を作り、入庫を台帳と監査ログに残す（トランザクション内）
- **修理**: 修理に出せるのは在庫あり・使用中で数量1以上の在庫のみ。修理は1個ずつで、数量2以上のロットからは1個を別行に切り出して修理の対象にする（残りは使える状態のまま）。修理の状態変更と在庫の状態変更は同一トランザクション
